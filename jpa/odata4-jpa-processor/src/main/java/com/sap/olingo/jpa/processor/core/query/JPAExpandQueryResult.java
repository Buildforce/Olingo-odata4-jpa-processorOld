        ...
        if (!jpaEntityType.getCollectionAttributesPath().isEmpty()) {
            ...
            if (SelectOptionUtil.selectAll(selectOptions)) {
                ...
                for (final SelectItem item : selectOptions.getSelectItems()) {
                    final String pathItem = item.getResourcePath().getUriResourceParts()
                            .stream().map(UriResource::getSegmentValue).collect(Collectors.joining(JPAPath.PATH_SEPARATOR));
